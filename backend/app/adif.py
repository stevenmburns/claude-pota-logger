...
def generate_adif(activation: Activation, qsos: list[QSO]) -> str:
    lines = []
    ...
    for qso in qsos:
        record = []
        record.append(_adif_field("STATION_CALLSIGN", activation.operator_callsign.upper()))
        record.append(_adif_field("MY_SIG", "POTA"))
        record.append(_adif_field("MY_SIG_INFO", activation.park_reference.upper()))
