...
from sqlalchemy import Date, DateTime, Float, ForeignKey, Integer, String, TypeDecorator, UniqueConstraint
...
class QSO(Base):
    ...
    hunt_session_id: Mapped[uuid.UUID] = mapped_column(
        SQLiteUUID(), ForeignKey("hunt_sessions.id")
    )
...
class Settings(Base):
    ...
    flrig_host: Mapped[str] = mapped_column(String(100), default="localhost")
